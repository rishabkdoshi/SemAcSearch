	...
	foreach($synonym[$token] as $syns)
	{
  	...
  	$query = mysql_query("SELECT 1 FROM ".$syns, $link);
		...
		if(!array_key_exists($row['PID'],$pageranks))
		{
			...
			$pageranks[$row['PID']] = $pageranks[$row['PID']] + PageRank('Title_c',$row['Title_c'],$weight/2);
			$pageranks[$row['PID']] = $pageranks[$row['PID']] + PageRank('Author_c',$row['Author_c'],$weight/2);
			...
			$pageranks[$row['PID']] = $pageranks[$row['PID']] + PageRank('Citation',$row2['CITATION_COUNT'],$weight/2);
		}
	}
	*/
}
